...
class WebformController extends Controller
{
    public function __construct(
        ...
        protected WebformPageSyncService $pageSync,
    ) {}
    ...
    public function store(StoreWebformRequest $request): RedirectResponse
    {
        ...
        $syncMessage = $this->syncPublicPage($webform);

        ActivityLogger::log('webforms', 'created', $webform);

        return redirect()
            ->route('admin.webforms.builder', $webform)
            ->with('success', 'Webform created. Add fields in the builder.'.$syncMessage);
    }
    ...
    public function update(UpdateWebformRequest $request, Webform $webform): RedirectResponse
    {
        ...
        return redirect()->route('admin.webforms.index')->with('success', 'Webform updated.'.$syncMessage);
    }
    ...
    public function destroy(Webform $webform): RedirectResponse
    {
        ...
        $this->pageSync->removeWebformPage($webform);
    ...
    protected function syncPublicPage(Webform $webform): string
    {
        $result = $this->pageSync->syncWebformPage($webform->fresh());
